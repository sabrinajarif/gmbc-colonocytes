...
```{r}
bestpenaltyX <- 0.1 #GENES
bestpenaltyY <- 0.07 #MICROBES # DECENT RESULTS

#bestpenaltyX <- 0.05 #GENES
#bestpenaltyY <- 0.05 #MICROBES # DECENT RESULTS
```
```{r}
...
```
```{r}
bestpenaltyX_urban <- 0.13 #GENES
bestpenaltyY_urban <- 0.07 #MICROBES

#bestpenaltyX_urban <- 0.05 #GENES
#bestpenaltyY_urban <- 0.05 #MICROBES

bestpenaltyX_rural <- 0.13 #GENES
bestpenaltyY_rural <- 0.07 #MICROBES

#bestpenaltyX_rural <- 0.05 #GENES
#bestpenaltyY_rural <- 0.05 #MICROBES
```
...
## run sparse CCA
cca_all <- run_sparseCCA(genes, microbes, cca.k, bestpenaltyX, bestpenaltyY,
                     outputFile=paste0(current_dir,"/results/docs/CCA/microbes/CCA_all_output_",bestpenaltyX,"_",bestpenaltyY,".txt"))
...
paste("avg.genes_all:", avg_genes_all)
paste("avg.genes_urban:", avg_genes_urban)
paste("avg.genes_rural:", avg_genes_rural)

avg.microbes_all <- get_avg_features(cca_all[[1]]$v, cca.k)
avg.microbes_urban <- get_avg_features(cca_urban[[1]]$v, cca.k)
avg.microbes_rural <- get_avg_features(cca_rural[[1]]$v, cca.k)
paste("avg.microbes_all:", avg.microbes_all)
paste("avg.microbes_urban: ", avg.microbes_urban)
paste("avg.microbes_rural: ", avg.microbes_rural)
```
...
CCA_pval_all <- test_significance_LOOCV(genes, microbes, bestpenaltyX, bestpenaltyY, cca.k)
...
dirname_all <- paste0(current_dir,"/results/docs/CCA/microbes/CCA_all_gene_taxa_components_",bestpenaltyX,"_",bestpenaltyY,"/")
...
# microbes only
sig_all_microbes <- lapply(sig_all_df, function(df) na.omit(df$taxa))
...
CCA_microbe_elements <- list(genes = genes,
                         ...
                         sig_all_microbes = sig_all_microbes,
...
saveRDS(CCA_microbe_elements, "results/rds/CCA_microbe_elements_all.rds")
```
```{r}
