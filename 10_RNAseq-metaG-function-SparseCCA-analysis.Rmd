...
CCA_function_elements <- readRDS("results/rds/CCA_microbe_elements_function_20240729.rds")
genes_urban = CCA_function_elements[["genes_urban"]]
genes_rural = CCA_function_elements[["genes_rural"]]
sig_urban_df = CCA_function_elements[["sig_urban_df"]]
sig_rural_df = CCA_function_elements[["sig_rural_df"]]
sig_urban_functions = CCA_function_elements[["sig_urban_pathways"]]
sig_rural_functions = CCA_function_elements[["sig_rural_pathways"]]
sig_urban_genes = CCA_function_elements[["sig_urban_genes"]]
sig_rural_genes = CCA_function_elements[["sig_rural_genes"]]

sig_urban_functions
```
Check if pathway coefficients are of the same direction
```{r}
# do all microbes have the same directionality?
check_directionality <- function(df) {
  # Remove NA values
  coeffs <- df$functions_coeff[!is.na(df$functions_coeff)]
...
determine_cor_dir <- function(df) {
  ...
  functions_coeffs <- df$functions_coeff[!is.na(df$functions_coeff)]
  
  # Determine the majority direction of taxa_coeffs
  majority_positive <- sum(functions_coeffs > 0) > sum(functions_coeffs < 0)
  ...
  for (i in 1:nrow(df)) {
    if (!is.na(df$functions_coeff[i])) {
      if ((majority_positive && df$functions_coeff[i] > 0) || (!majority_positive && df$functions_coeff[i] < 0)) {
        df$cor_dir[i] <- "pos"
      }
    }
  }
...
head(sig_urban_df_dir[[3]])
```
Split into pos and neg correlations
```{r}
sig_urban_df_dir_pos <- lapply(sig_urban_df_dir, function(df) {df %>% dplyr::filter(cor_dir == "pos")})
sig_urban_df_dir_neg <- lapply(sig_urban_df_dir, function(df) {df %>% dplyr::filter(cor_dir == "neg")})
sig_rural_df_dir_pos <- lapply(sig_rural_df_dir, function(df) {df %>% dplyr::filter(cor_dir == "pos")})
sig_rural_df_dir_neg <- lapply(sig_rural_df_dir, function(df) {df %>% dplyr::filter(cor_dir == "neg")})
...
annotate_ensembl <- function(ensembl_ids) {
  ensembl = useEnsembl(biomart = "ensembl", dataset = "hsapiens_gene_ensembl", host = "jul2023.archive.ensembl.org", mirror = "www")
...
enriched_pathways_urban <- enriched_pathways_func(NULL, u_neg_enrich_sig) #u_pos_enrich_sig
enriched_pathways_rural <- enriched_pathways_func(NULL, r_neg_enrich_sig) #r_pos_enrich_sig
...
ggplot(enriched_pathways_urban, aes(x=1, y=ID_3, size=GeneRatio_numeric, color=dir)) + 
  geom_point() +
  scale_color_manual(values=c("tomato")) + #blue is pos orange is neg
  ...
  guides(alpha = guide_legend(title = "-log10 p.adjust"), 
         size = guide_legend(title = "Gene Ratio"))

#ggsave("results/figs/RNAseq-metaG-function-SparseCCA/enriched_pathways_urban_20250328.png", width=4.2, height=1.5)


ggplot(enriched_pathways_rural, aes(x=1, y=ID_3, size=GeneRatio_numeric, color=dir)) + 
  geom_point() +
  scale_color_manual(values=c("tomato")) + #blue is pos orange is neg
  scale_y_discrete(position = "right") +
  facet_grid(component~ ., scales = "free_y", space = "free_y") +
...
#ggsave("results/figs/RNAseq-metaG-function-SparseCCA/enriched_pathways_rural_20250328.png", width=4.2, height=3)
```
