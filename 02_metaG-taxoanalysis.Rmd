...
#aldex2_cts <- function(aldex_obj){
#  x.counts_obj <- getMonteCarloInstances(aldex_obj)
#  x.counts <- as.data.frame(x.counts_obj$reads) 
#  return(x.counts)
#}
```
```{r}
covariates <- phyloseq::sample_data(phy_filt_g) %>%
  data.frame() %>%
  dplyr::select(urbanism, country)
mm <- model.matrix(~ urbanism, covariates)
x.glm <- aldex.clr(as.data.frame(otu_table(phy_filt_g)), mm, mc.samples=128, denom="all", verbose=T)
...
ggplot(filter(test, OTU=="Bacteroides_finegoldii"), aes(x=urbanism, y=CLR_abundance, fill=urbanism)) +
  geom_boxplot() +
  scale_fill_manual(values=c("#6FBD8B", "#7B2BC1")) + 
  theme_linedraw() + 
  theme_linedraw() +
  labs(x = "Bacteroides finegoldii", y = NULL) +  # Remove x and y axis titles
  theme(
...
ggsave("results/figs/RNAseq-metaG-microbe-SparseCCA/Bacteroides_finegoldii.png", width=2, height=1.5)
