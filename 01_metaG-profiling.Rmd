...
get raw/multiqc_report.html /Users/sabri/Downloads/multiqc_report_raw.html
get post_cutadapt/multiqc_report.html /Users/sabri/Downloads/multiqc_report_cutadapt.html
get post_kneaddata/multiqc_report.html /Users/sabri/Downloads/multiqc_report_kneaddata.html
EOF
```
...
get bracken_species_combined.txt /Users/sabri/Downloads/bracken_species_combined.txt
get taxonomy.tsv /Users/sabri/Downloads/taxonomy.tsv
EOF
```
Peek at data
```{r}
...
bracken_species_combined <- read.csv("~/Downloads/bracken_species_combined.txt", sep="\t")
head(bracken_species_combined)

taxonomy <- read.csv("~/Downloads/taxonomy.tsv", sep="\t", header=F)
# This messy solution is required because of the bad strings
taxonomy_sep <- taxonomy %>%
  mutate(row = V1) %>%
  column_to_rownames("row") %>%
  rename(taxonomy_id=2) %>%
  select(taxonomy_id, everything()) %>%
...
I THINK I CAN USE KRAKEN/BRACKEN INPUT BUT IT MIGHT BE MORE COMPLICATED THAN I THOUGHT. NEED TO CREATE CUSTOM NUC AND PROT DATABASES IN THE STYLE OF METAPHLAN (MPA)
```{bash, eval=F}
...
IN_DIR_kraken=/scratch.global/sarif/2023_GMbC_metaG/tax_profiling/kraken/p1p2_combined
IN_DIR_bracken=/scratch.global/sarif/2023_GMbC_metaG/tax_profiling/bracken/p1p2_combined
OUT_DIR=/scratch.global/sarif/2023_GMbC_metaG/func_profiling
REF_DIR_nuc=/home/blekhman/sarif/gmbc_ccyte/KrakenBrackenHUMAnN/humann_dbs/chocophlan
REF_DIR_prot=/home/blekhman/sarif/gmbc_ccyte/KrakenBrackenHUMAnN/humann_dbs/uniref
...
for i in *.fastq
do
  humann --input-format kraken --kraken $IN_DIR_kraken/$i.kraken --bracken $IN_DIR_bracken/$i.bracken --output $OUT_DIR --nucleotide-database $REF_DIR_nuc --protein-database $REF_DIR_prot --threads 32 --translated-identity-threshold 97 --prescreen-threshold 0.0001 --nucleotide-query-coverage-threshold 70 --translated-query-coverage-threshold 97 --diamond-options="--threads 32"
done
```
