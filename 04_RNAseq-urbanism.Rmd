...
enriched_pathways <- rbind(uc_enrich_res, rc_enrich_res) %>%
  ...
  mutate(sig = case_when(
    model=="uc" & ID %in% uc_enrich_sig$ID ~ "sig_uc",
    model=="rc" & ID %in% rc_enrich_sig$ID ~ "sig_rc",
    model=="uc" & !ID %in% uc_enrich_sig$ID ~ "ns_uc",
    model=="rc" & !ID %in% rc_enrich_sig$ID ~ "ns_rc",
  )) %>%
  dplyr::filter(!sig=="ns_uc", !sig=="ns_rc") %>%
  mutate(sig_in = case_when(
    ID %in% uc_enrich_sig$ID & ID %in% rc_enrich_sig$ID ~ "both",
    ID %in% uc_enrich_sig$ID & !ID %in% rc_enrich_sig$ID ~ "urban_only",
    ID %in% rc_enrich_sig$ID & !ID %in% uc_enrich_sig$ID ~ "rural_only",
    TRUE ~ "Other"
  )) %>%
...
ggplot(enriched_pathways, aes(x=GeneRatio_numeric, y=ID_2, fill=model, color=model, size=neglog10_p.adjust)) + #, size=neglog10_p.adjust # shape=sig
  geom_point() +
...
ggsave("results/figs/RNAseq-urbanism/enriched_pathways.png", width=6, height=3)
```
